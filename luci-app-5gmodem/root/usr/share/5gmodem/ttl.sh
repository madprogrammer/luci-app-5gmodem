...
apply)
	...
	mkdir -p /etc/nftables.d
	{
		...
		# Standalone base chains only: fw4 places /etc/nftables.d includes
		# BEFORE its own chain definitions, so re-declaring fw4 chain names
		# (mangle_prerouting etc.) creates hookless chains that make the
		# whole ruleset fail to load on the next boot/reload.
		if [ -n "$DEV" ]; then
			if [ -n "$T4I$T6I" ]; then
				echo "chain ttl5g_prerouting {"
				echo "	type filter hook prerouting priority -160; policy accept;"
				[ -n "$T4I" ] && echo "	iifname \"$DEV\" ip ttl set $T4I;"
				[ -n "$T6I" ] && echo "	iifname \"$DEV\" ip6 hoplimit set $T6I;"
				echo "}"
			fi
			if [ -n "$T4O$T6O" ]; then
				echo "chain ttl5g_postrouting {"
				echo "	type filter hook postrouting priority -160; policy accept;"
				[ -n "$T4O" ] && echo "	oifname \"$DEV\" ip ttl set $T4O;"
				[ -n "$T6O" ] && echo "	oifname \"$DEV\" ip6 hoplimit set $T6O;"
				echo "}"
			fi
		fi
	} > "$NFT"
	...
	# never leave an include behind that breaks the whole fw4 ruleset load
	if [ -f "$NFT" ] && ! fw4 check >/dev/null 2>&1; then
		rm -f "$NFT"
		fw4 reload >/dev/null 2>&1
		echo "ERROR: generated TTL include failed fw4 check - removed" >&2
		exit 1
	fi
